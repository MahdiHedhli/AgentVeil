...
| AV-003 | Audit, dashboard, generated report, automated output, or unintended terminal output contains originals, mappings, auth, or tokens. | Closed typed audit schema; no body logging; audit unit/file/wire/live scans; dashboard consumes typed audit metadata and has local-asset/no-secret tests. The only approved original-value display is the explicitly interactive synthetic `codex-demo` TUI, using its two documented lower-risk fixtures. | **Controlled for the current candidate; recurring.** Clean release/report scans must remain value-free and the public capture needs separate visual review. |
...
| AV-005 | Restored originals persist in or replay from local Codex threads. | Gateway rejects restoration with OpenAI; live responses remain tokenized. The synthetic Codex demo isolates its resumable thread, removes and verifies the root on clean exit, and documents forced-kill/crash residue. | **Open research / safely disabled for live OpenAI.** Synthetic demo uses shipped fixtures only; do not enable live restoration until transcript lifecycle and replay are independently proven. |
...
| AV-025 | Release dashboard/report/demo claims outrun the core evidence. | Packaged demos assert capturing-upstream state; live dashboard says wire proof is not measured; synthetic failure is sticky; value-free dashboard state v2 adds client and expanded restoration/wire-proof labels; release reports bind status to commit and clean-tree state. | **Controlled for the current candidate.** Repeat clean release/leak, benchmark, both synthetic checks, fake-wire, and live synthetic gates after any source change. |
